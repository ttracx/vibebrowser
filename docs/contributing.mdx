---
title: "Contributing to VibeBrowser"
...
<CardGroup cols={3}>
  ...
  <Card
    title="Report Issues"
    icon="bug"
    href="https://github.com/ttracx/vibebrowser/issues"
  >
...
You can contribute to VibeBrowser in many ways! Whether you want to build features or help out in other ways, we appreciate all contributions.
...
<Warning>
**Note:** The `packages/vibebrowser-agent` directory is not yet available in this repository. Agent development instructions will be updated once the agent code is added to the monorepo.
</Warning>

<Accordion title="Instructions (Coming Soon)">
...
<Steps>
  ...
  <Step title="Load Unpacked Extension">
    Click **Load unpacked** and select `packages/vibebrowser-agent/dist/`
  </Step>
...
<Note>
For detailed setup, architecture, and code standards, see the [Agent Contributing Guide](https://github.com/ttracx/vibebrowser/blob/main/packages/vibebrowser-agent/CONTRIBUTING.md).
</Note>
...
<Steps>
  ...
  <Step title="Run VibeBrowser">
    After the build completes successfully, you can run VibeBrowser:

    <Tabs>
      <Tab title="macOS Debug (ARM64)">
        ```bash
        out/Default_arm64/VibeBrowser\ Dev.app/Contents/MacOS/VibeBrowser\ Dev --user-data-dir=/tmp/test-profile
        ```
      </Tab>
      <Tab title="macOS Debug (x64)">
        ```bash
        out/Default_x64/VibeBrowser\ Dev.app/Contents/MacOS/VibeBrowser\ Dev --user-data-dir=/tmp/test-profile
        ```
      </Tab>
      <Tab title="macOS Release (ARM64)">
        ```bash
        out/Default_arm64/VibeBrowser.app/Contents/MacOS/VibeBrowser --user-data-dir=/tmp/test-profile
        ```
      </Tab>
      <Tab title="macOS Release (x64)">
        ```bash
        out/Default_x64/VibeBrowser.app/Contents/MacOS/VibeBrowser --user-data-dir=/tmp/test-profile
        ```
      </Tab>
...
<Steps>
  <Step title="Read the CLA">
    Read the [CLA document](https://github.com/VibeBrowser/VibeBrowser/blob/main/CLA.md)
  </Step>
...
- Agent: [packages/vibebrowser-agent/CLAUDE.md](https://github.com/ttracx/vibebrowser/blob/main/packages/vibebrowser-agent/CLAUDE.md)
</Note>
...
│   │   ├── chromium_files/    # Chromium branding files
│   │   ├── patches/           # VibeBrowser patches
│   │   └── resources/         # Icons, configs
│   │
│   └── vibebrowser-agent/        # Chrome extension (coming soon)
│
...
<Note>
**Current Status:** The repository currently contains the Chromium browser patches and resources. The agent extension and Python build system will be added in future updates.
</Note>
...
<Steps>
  <Step title="Fork and Clone">
    ...
    ```bash
    git clone https://github.com/YOUR-USERNAME/vibebrowser.git
    cd vibebrowser
    ```
  </Step>
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    icon="comments"
    href="https://github.com/ttracx/vibebrowser/discussions"
  >
    General questions
  </Card>
</CardGroup>
