...
description: "VibeBrowser works best with your own API keys!"
---

To use VibeBrowser, you need to configure an LLM provider. You have two options:

1. **Use models locally** - Connect VibeBrowser to models running on your machine
2. **Bring your own API keys** - Use cloud providers like ChatGPT, Gemini, or Claude

Visit `chrome://settings/VibeBrowser` to configure your LLM provider.

<Note>
  **Our recommendation:** VibeBrowser works best with **Gemini 2.5 Flash**. We highly recommend getting an API key from Google AI Studio for Gemini 2.5 Flash. **Google AI Studio gives free access to Gemini 2.5 Flash models for up to 20 requests per minute** - perfect for getting started without any costs.
</Note>
...
<AccordionGroup>
  <Accordion title="🔒 Privacy - You control your data">
    Your API keys are stored locally and encrypted. Requests go directly from your browser to the provider - VibeBrowser servers never see your data or keys. With local models, your data never even leaves your machine.
  </Accordion>
  <Accordion title="⚡ Rate limits - Avoid usage restrictions">
    VibeBrowser has heavy rate limits on the default shared models. For the smoothest experience without interruptions, bring your own API keys or run models locally. Use VibeBrowser as much as you want without hitting limits.
  </Accordion>
</AccordionGroup>

## Option #1: Bring your own keys to ChatGPT, Gemini, Claude! (Recommended)

Connect to powerful cloud models using your own API keys. VibeBrowser works very well with Gemini 2.5 Flash - use it for the highest performance, fastest speed, and most stable experience.
...
<Warning>
  Many models like Deepseek-R1 via Ollama don't support tool calls and you'll get `400` errors when trying to use them with VibeBrowser. Stick with the recommended models above for the best experience.
</Warning>
...
Once you've configured your LLM provider, explore what VibeBrowser can do:
...
- Use **Teach Mode** to create repeatable workflows—show the VibeBrowser agent your workflow once, and it will learn and repeat it reliably.
---
<p align="center">
