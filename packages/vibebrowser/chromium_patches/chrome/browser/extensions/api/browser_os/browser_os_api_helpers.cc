...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[VibeBrowser] No RenderFrameHost for AccessibilityDoDefault";
+    return false;
+  }
+  
...
+  
+  LOG(INFO) << "[VibeBrowser] Performing AccessibilityDoDefault on node " 
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[VibeBrowser] No RenderFrameHost for AccessibilityFocus";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  action_data.target_tree_id = node_info.ax_tree_id;
+  
+  LOG(INFO) << "[VibeBrowser] Performing AccessibilityFocus on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[VibeBrowser] No RenderFrameHost for AccessibilityScrollToMakeVisible";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[VibeBrowser] Performing AccessibilityScrollToMakeVisible on node " 
+            << node_info.ax_node_id;
+  
+  rfh->AccessibilityPerformAction(action_data);
+  return true;
...
+    LOG(INFO) << "[VibeBrowser] Node is out of viewport, scrolling to make visible";
...
+    // LOG(INFO) << "[VibeBrowser] Node was out of viewport, trying AccessibilityDoDefault click first";
+    // bool changed = VibeBrowserChangeDetector::ExecuteWithDetection(
+    //     web_contents,
...
+      LOG(INFO) << "[VibeBrowser] No change from accessibility click, trying HTML click";
+      changed = VibeBrowserChangeDetector::ExecuteWithDetection(
+          web_contents,
+          [&]() { HtmlClick(web_contents, node_info); },
+          base::Milliseconds(200));
+    }
+    
+    LOG(INFO) << "[VibeBrowser] Click result: " << (changed ? "changed" : "no change");
+    return changed;
+  }
+  
+  // For in-viewport nodes, try coordinate click first (most natural)
+  LOG(INFO) << "[VibeBrowser] Node is in viewport, trying coordinate click first";
+  gfx::PointF click_point = GetNodeCenterPoint(web_contents, node_info);
+  
+  bool changed = VibeBrowserChangeDetector::ExecuteWithDetection(
+      web_contents,
...
+  content::RenderFrameHost* rfh = web_contents->GetPrimaryMainFrame();
+  if (!rfh) {
+    LOG(WARNING) << "[VibeBrowser] No RenderFrameHost for AccessibilitySetValue";
+    return false;
+  }
+  
+  ui::AXActionData action_data;
...
+  
+  LOG(INFO) << "[VibeBrowser] Performing AccessibilitySetValue on node " 
...
+  
+  if (is_out_of_viewport) {
+    LOG(INFO) << "[VibeBrowser] Node is out of viewport for typing, scrolling to make visible";
+    AccessibilityScrollToMakeVisible(web_contents, node_info, true /* center */);
+    // Wait for scroll to complete
+    base::PlatformThread::Sleep(base::Milliseconds(300));
+  }
...
+  if (!changed) {
+    LOG(INFO) << "[VibeBrowser] No change from native typing, trying JavaScript";
+    changed = VibeBrowserChangeDetector::ExecuteWithDetection(
+        web_contents,
...
+  // if (!changed) {
+  //   LOG(INFO) << "[VibeBrowser] No change from JavaScript, trying accessibility SetValue";
+  //   changed = VibeBrowserChangeDetector::ExecuteWithDetection(
+  //       web_contents,
+  //       [&]() {
...
+  // }
+  
+  LOG(INFO) << "[VibeBrowser] Type result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[VibeBrowser] Clear result: " << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+      base::Milliseconds(200));
+  
+  LOG(INFO) << "[VibeBrowser] KeyPress result for '" << key << "': " 
...
+    LOG(INFO) << "[VibeBrowser] No interactive elements in viewport to highlight";
+    return;
+  }
+  
+  LOG(INFO) << "[VibeBrowser] Highlighting " << filtered_nodes.size() 
...
+      document.querySelectorAll('.VibeBrowser-bbox').forEach(e => e.remove());
+      
+      // Create container for all bounding boxes
+      const container = document.createElement('div');
+      container.className = 'VibeBrowser-bbox-container';
...
+        box.className = 'VibeBrowser-bbox';
...
+      document.querySelectorAll('.VibeBrowser-bbox-container').forEach(e => e.remove());
+      document.querySelectorAll('.VibeBrowser-bbox').forEach(e => e.remove());
+      
+      // Remove all highlight containers  
+      document.querySelectorAll('.VibeBrowser-highlight-container').forEach(e => e.remove());
+      document.querySelectorAll('.VibeBrowser-highlight').forEach(e => e.remove());
+      
+      // Remove any style elements we added
+      document.querySelectorAll('#VibeBrowser-highlight-styles').forEach(e => e.remove());
+      
+      return true;
+    })();
+  )";
...
+                                   const gfx::PointF& point) {
+  LOG(INFO) << "[VibeBrowser] ClickCoordinatesWithDetection at (" 
+            << point.x() << ", " << point.y() << ")";
+  
...
+      base::Milliseconds(300));
+  
+  LOG(INFO) << "[VibeBrowser] Click coordinates result: " 
+            << (changed ? "changed" : "no change");
+  return changed;
+}
+
...
+                                    const gfx::PointF& point,
+                                    const std::string& text) {
+  LOG(INFO) << "[VibeBrowser] TypeAtCoordinatesWithDetection at (" 
...
+  if (!changed) {
+    LOG(INFO) << "[VibeBrowser] No change from native typing at coordinates, trying JS injection";
+    
...
+  }
+  
+  LOG(INFO) << "[VibeBrowser] Type at coordinates result: " 
