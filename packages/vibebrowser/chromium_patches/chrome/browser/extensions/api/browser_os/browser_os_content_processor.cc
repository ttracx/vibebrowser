...
+    LOG(INFO) << "VibeBrowser: ExtractPageContent - tree is empty";
+    return items;
+  }
+
+  LOG(INFO) << "VibeBrowser: ExtractPageContent - processing " << tree_update.nodes.size() << " nodes";
+
...
+
+  LOG(INFO) << "VibeBrowser: ExtractPageContent - extracted " << items.size() << " items";
+
+  return items;
+}
+
