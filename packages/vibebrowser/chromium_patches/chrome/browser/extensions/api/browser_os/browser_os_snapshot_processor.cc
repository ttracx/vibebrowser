...
+            &is_offscreen);
+        
+        VLOG(3) << "[VibeBrowser] Node " << node_data.id 
...
+    } else {
+      // No AXTree available
+      LOG(WARNING) << "[VibeBrowser] No AXTree available for bounds computation";
+    }
+    
...
+  }
+  
+  LOG(INFO) << "[VibeBrowser] Viewport: " << viewport_size.ToString() 
...
+  
+  if (!ax_tree) {
+    LOG(ERROR) << "[VibeBrowser] Failed to create AXTree from update";
+    SnapshotProcessingResult result;
...
+  }
+  
+  LOG(INFO) << "[VibeBrowser] Created AXTree with " << tree_update.nodes.size() 
