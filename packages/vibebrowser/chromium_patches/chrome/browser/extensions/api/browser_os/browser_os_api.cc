...
+uint32_t VibeBrowserGetInteractiveSnapshotFunction::next_snapshot_id_ = 1;
+
+// Constructor and destructor implementations
+VibeBrowserGetInteractiveSnapshotFunction::VibeBrowserGetInteractiveSnapshotFunction() = default;
+VibeBrowserGetInteractiveSnapshotFunction::~VibeBrowserGetInteractiveSnapshotFunction() = default;
+
+ExtensionFunction::ResponseAction VibeBrowserGetAccessibilityTreeFunction::Run() {
...
+      base::BindOnce(
+          &VibeBrowserGetAccessibilityTreeFunction::OnAccessibilityTreeReceived,
+          this),
...
+}
+
+void VibeBrowserGetAccessibilityTreeFunction::OnAccessibilityTreeReceived(
...
+}
+
+// Implementation of VibeBrowserGetInteractiveSnapshotFunction
+
+ExtensionFunction::ResponseAction VibeBrowserGetInteractiveSnapshotFunction::Run() {
...
+    LOG(WARNING) << "[VibeBrowser] Frame not stable for AX snapshot - skipping";
...
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(
+          &VibeBrowserGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived,
+          this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties |
+                 ui::AXMode::kInlineTextBoxes),
+      /* max_nodes= */ 0,  // No limit
...
+}
+
+void VibeBrowserGetInteractiveSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  // Double-check frame is still valid before processing
+  if (!web_contents_) {
+    LOG(WARNING) << "[VibeBrowser] WebContents gone during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+    LOG(WARNING) << "[VibeBrowser] Frame became unstable during AX snapshot callback";
+    browser_os::InteractiveSnapshot empty_snapshot;
+    empty_snapshot.snapshot_id = next_snapshot_id_++;
+    empty_snapshot.timestamp = base::Time::Now().InMillisecondsFSinceUnixEpoch();
+    empty_snapshot.processing_time_ms = 0;
...
+      web_contents_,
+      base::BindOnce(
+          &VibeBrowserGetInteractiveSnapshotFunction::OnSnapshotProcessed,
+          base::WrapRefCounted(this)));
+}
+
+void VibeBrowserGetInteractiveSnapshotFunction::OnSnapshotProcessed(
+    SnapshotProcessingResult result) {
+  Respond(ArgumentList(
+      browser_os::GetInteractiveSnapshot::Results::Create(result.snapshot)));
+}
+
+// Implementation of VibeBrowserClickFunction
+
+ExtensionFunction::ResponseAction VibeBrowserClickFunction::Run() {
...
+}
+
+// Implementation of VibeBrowserInputTextFunction
+
+ExtensionFunction::ResponseAction VibeBrowserInputTextFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[VibeBrowser] InputText: Starting input for nodeId: " << params->node_id;
+  
...
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[VibeBrowser] InputText: No change detected after typing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of VibeBrowserClearFunction
+
+ExtensionFunction::ResponseAction VibeBrowserClearFunction::Run() {
...
+  const NodeInfo& node_info = node_it->second;
+  
+  LOG(INFO) << "[VibeBrowser] Clear: Clearing field for nodeId: " << params->node_id;
+  
+  // Use ClearWithDetection which handles focus and clearing
+  bool change_detected = ClearWithDetection(web_contents, node_info);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[VibeBrowser] Clear: No change detected after clearing";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of VibeBrowserGetPageLoadStatusFunction
+
+ExtensionFunction::ResponseAction VibeBrowserGetPageLoadStatusFunction::Run() {
...
+}
+
+// Implementation of VibeBrowserScrollUpFunction
+
+ExtensionFunction::ResponseAction VibeBrowserScrollUpFunction::Run() {
...
+}
+
+// Implementation of VibeBrowserScrollDownFunction
+
+ExtensionFunction::ResponseAction VibeBrowserScrollDownFunction::Run() {
...
+}
+
+// Implementation of VibeBrowserScrollToNodeFunction
+
+ExtensionFunction::ResponseAction VibeBrowserScrollToNodeFunction::Run() {
...
+}
+
+// Implementation of VibeBrowserSendKeysFunction
+
+ExtensionFunction::ResponseAction VibeBrowserSendKeysFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[VibeBrowser] SendKeys: Sending key '" << params->key << "'";
+  
+  // Send the key with change detection
+  bool change_detected = KeyPressWithDetection(web_contents, params->key);
+  
+  if (!change_detected) {
+    LOG(WARNING) << "[VibeBrowser] SendKeys: No change detected after key press";
+  }
+  
+  // Create interaction response
+  browser_os::InteractionResponse response;
...
+}
+
+// Implementation of VibeBrowserCaptureScreenshotFunction
+
+VibeBrowserCaptureScreenshotFunction::VibeBrowserCaptureScreenshotFunction() = default;
+VibeBrowserCaptureScreenshotFunction::~VibeBrowserCaptureScreenshotFunction() = default;
+
+ExtensionFunction::ResponseAction VibeBrowserCaptureScreenshotFunction::Run() {
...
+    LOG(INFO) << "[VibeBrowser] CaptureScreenshot: Using exact dimensions: "
...
+      LOG(INFO) << "[VibeBrowser] CaptureScreenshot: Using thumbnail size: " << max_dimension 
...
+      LOG(INFO) << "[VibeBrowser] CaptureScreenshot: Using viewport size: " << max_dimension;
+    }
+    
...
+}
+
+void VibeBrowserCaptureScreenshotFunction::DrawHighlightsAndCapture() {
...
+      LOG(INFO) << "[VibeBrowser] Drawing highlights for screenshot with " 
+                << tab_it->second.size() << " interactive elements";
+      ShowHighlights(web_contents_, tab_it->second, true /* show_labels */);
+    } else {
+      LOG(INFO) << "[VibeBrowser] No snapshot data available for highlighting";
+    }
+    
...
+        FROM_HERE,
+        base::BindOnce(&VibeBrowserCaptureScreenshotFunction::CaptureScreenshotNow,
+                       base::WrapRefCounted(this)),
...
+}
+
+void VibeBrowserCaptureScreenshotFunction::CaptureScreenshotNow() {
+  if (!web_contents_) {
...
+      target_size_,
+      base::BindOnce(&VibeBrowserCaptureScreenshotFunction::OnScreenshotCaptured,
+                     this));
+}
+
+void VibeBrowserCaptureScreenshotFunction::OnScreenshotCaptured(
...
+}
+
+// VibeBrowserGetSnapshotFunction implementation
+ExtensionFunction::ResponseAction VibeBrowserGetSnapshotFunction::Run() {
...
+  // Request accessibility tree snapshot
+  web_contents->RequestAXTreeSnapshot(
+      base::BindOnce(&VibeBrowserGetSnapshotFunction::OnAccessibilityTreeReceived,
+                     this),
+      ui::AXMode(ui::AXMode::kWebContents | ui::AXMode::kExtendedProperties),
+      /* max_nodes= */ 0,  // No limit
+      /* timeout= */ base::TimeDelta(),
...
+}
+
+void VibeBrowserGetSnapshotFunction::OnAccessibilityTreeReceived(
+    ui::AXTreeUpdate& tree_update) {
+  if (!has_callback()) {
+    return;
+  }
...
+}
+
+// VibeBrowserGetPrefFunction
+ExtensionFunction::ResponseAction VibeBrowserGetPrefFunction::Run() {
...
+}
+
+// VibeBrowserSetPrefFunction
+ExtensionFunction::ResponseAction VibeBrowserSetPrefFunction::Run() {
+  std::optional<browser_os::SetPref::Params> params =
+      browser_os::SetPref::Params::Create(args());
+  EXTENSION_FUNCTION_VALIDATE(params);
+
+  // Security: only allow modifying VibeBrowser.* prefs
+  if (!params->name.starts_with("VibeBrowser.")) {
+    return RespondNow(Error("Only VibeBrowser.* preferences can be modified"));
+  }
+
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* prefs = FindPrefService(params->name, profile);
...
+}
+
+// VibeBrowserGetAllPrefsFunction
+ExtensionFunction::ResponseAction VibeBrowserGetAllPrefsFunction::Run() {
+  Profile* profile = Profile::FromBrowserContext(browser_context());
+  PrefService* profile_prefs = profile->GetPrefs();
+  PrefService* local_state = g_browser_process->local_state();
+
+  // Build a combined VibeBrowser prefs dict from both sources
+  base::Value::Dict combined_VibeBrowser;
+
+  // Lambda to merge VibeBrowser prefs from a PrefService
...
+
+    // Look for "VibeBrowser" key in the top-level dict
+    const base::Value* VibeBrowser_value = pref_dict.Find("VibeBrowser");
+    if (VibeBrowser_value && VibeBrowser_value->is_dict()) {
+      // Merge this VibeBrowser dict into combined
+      combined_VibeBrowser.Merge(VibeBrowser_value->GetDict().Clone());
+      LOG(INFO) << "[VibeBrowser] GetAllPrefs: Found VibeBrowser.* prefs in " << source_name;
+    }
+  };
+
...
+
+  // Create single PrefObject with the entire VibeBrowser dict
+  std::vector<browser_os::PrefObject> pref_objects;
+  browser_os::PrefObject pref_obj;
+  pref_obj.key = "VibeBrowser";
+  pref_obj.type = "dictionary";
+  pref_obj.value = base::Value(std::move(combined_VibeBrowser));
...
+}
+
+// VibeBrowserLogMetricFunction
+ExtensionFunction::ResponseAction VibeBrowserLogMetricFunction::Run() {
...
+    
+    VibeBrowser_metrics::VibeBrowserMetrics::Log(prefixed_event, std::move(properties));
+  } else {
+    // No properties, just log with extension ID
+    VibeBrowser_metrics::VibeBrowserMetrics::Log(prefixed_event, {
...
+}
+
+// VibeBrowserGetVersionNumberFunction
+ExtensionFunction::ResponseAction VibeBrowserGetVersionNumberFunction::Run() {
...
+}
+
+// VibeBrowserExecuteJavaScriptFunction
+ExtensionFunction::ResponseAction VibeBrowserExecuteJavaScriptFunction::Run() {
...
+  }
+  
+  LOG(INFO) << "[VibeBrowser] ExecuteJavaScript: Executing code in tab " << tab_info->tab_id;
+  
...
+      js_code,
+      base::BindOnce(&VibeBrowserExecuteJavaScriptFunction::OnJavaScriptExecuted,
+                     this),
...
+}
+
+void VibeBrowserExecuteJavaScriptFunction::OnJavaScriptExecuted(base::Value result) {
+  LOG(INFO) << "[VibeBrowser] ExecuteJavaScript: Execution completed";
+
+  if (result.is_none()) {
...
+}
+
+// Implementation of VibeBrowserClickCoordinatesFunction
+ExtensionFunction::ResponseAction VibeBrowserClickCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[VibeBrowser] ClickCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
...
+  
+  LOG(INFO) << "[VibeBrowser] ClickCoordinates: Clicking at (" 
+            << params->x << ", " << params->y << ")";
+  
...
+  
+  LOG(INFO) << "[VibeBrowser] ClickCoordinates: Result = " 
...
+}
+
+// Implementation of VibeBrowserTypeAtCoordinatesFunction  
+ExtensionFunction::ResponseAction VibeBrowserTypeAtCoordinatesFunction::Run() {
...
+                                       &error_message);
+  if (!tab_info) {
+    LOG(ERROR) << "[VibeBrowser] TypeAtCoordinates: " << error_message;
+    browser_os::InteractionResponse response;
+    response.success = false;
+    return RespondNow(ArgumentList(
...
+  gfx::PointF click_point(params->x, params->y);
+  
+  LOG(INFO) << "[VibeBrowser] TypeAtCoordinates: Clicking at (" 
...
+  response.success = success;
+  
+  LOG(INFO) << "[VibeBrowser] TypeAtCoordinates: Result = " 
