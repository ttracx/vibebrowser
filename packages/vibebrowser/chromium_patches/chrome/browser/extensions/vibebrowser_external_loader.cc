diff --git a/chrome/browser/extensions/VibeBrowser_external_loader.cc b/chrome/browser/extensions/VibeBrowser_external_loader.cc
new file mode 100644
index 0000000000000..53eeebf1be954
--- /dev/null
+++ b/chrome/browser/extensions/VibeBrowser_external_loader.cc
@@ -0,0 +1,661 @@
...
+namespace {
+
+// Default config URL - should be updated to actual VibeBrowser server
+// Can be overridden via --VibeBrowser-extensions-url command line flag
+constexpr char kVibeBrowserConfigUrl[] = "https://cdn.VibeBrowser.com/extensions/extensions.json";
+
...
+constexpr net::NetworkTrafficAnnotationTag kVibeBrowserExtensionsFetchTrafficAnnotation =
+    net::DefineNetworkTrafficAnnotation("VibeBrowser_extensions_fetch", R"(
+        semantics {
+          sender: "VibeBrowser External Extension Loader"
+          description:
+            "Fetches a JSON configuration file that specifies which extensions "
+            "should be installed for VibeBrowser users at startup."
+          trigger:
+            "Triggered during browser startup when VibeBrowser mode is enabled."
+          data:
+            "No user data is sent. Only a GET request to fetch the configuration."
+          destination: OTHER
+          destination_other:
+            "The VibeBrowser configuration server specified by the config URL."
+        }
+        policy {
+          cookies_allowed: NO
+          setting:
...
+            "Not implemented yet. This is a new feature for VibeBrowser."
+        })");
+
+// Example JSON format:
+// {
...
+}  // namespace
+
+VibeBrowserExternalLoader::VibeBrowserExternalLoader(Profile* profile)
+    : profile_(profile) {
+  // Default config URL - can be overridden via SetConfigUrl
+  config_url_ = GURL(kVibeBrowserConfigUrl);
+  
+  // Add known VibeBrowser extension IDs
+  for (const char* extension_id : VibeBrowser::kAllowedExtensions) {
+    VibeBrowser_extension_ids_.insert(extension_id);
+  }
+}
+
+VibeBrowserExternalLoader::~VibeBrowserExternalLoader() = default;
+
+void VibeBrowserExternalLoader::StartLoading() {
+  LOG(INFO) << "VibeBrowser external extension loader starting...";
+  
...
+    LOG(ERROR) << "Invalid VibeBrowser extensions config URL";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+  
+  LOG(INFO) << "Fetching VibeBrowser extensions from: " << config_url_.spec();
+
...
+      std::move(resource_request), kVibeBrowserExtensionsFetchTrafficAnnotation);
+
+  // Start the download
+  url_loader_->DownloadToStringOfUnboundedSizeUntilCrashAndDie(
+      url_loader_factory_.get(),
+      base::BindOnce(&VibeBrowserExternalLoader::OnURLFetchComplete,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void VibeBrowserExternalLoader::OnURLFetchComplete(
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(ERROR) << "Failed to fetch VibeBrowser extensions config from " 
+               << config_url_.spec();
+    LoadFinished(base::Value::Dict());
+    return;
+  }
...
+}
+
+void VibeBrowserExternalLoader::ParseConfiguration(
+    const std::string& json_content) {
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(json_content);
+  
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(ERROR) << "Failed to parse VibeBrowser extensions config JSON";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  
+  if (!extensions_dict) {
+    LOG(ERROR) << "No 'extensions' key found in VibeBrowser config";
+    LoadFinished(base::Value::Dict());
+    return;
+  }
+
...
+  }
+  
+  LOG(INFO) << "Loaded " << prefs.size() << " extensions from VibeBrowser config";
+  
...
+  
+  // Immediately trigger high-priority installation of all VibeBrowser extensions
...
+  if (!VibeBrowser_extension_ids_.empty()) {
+    LOG(INFO) << "VibeBrowser: Triggering immediate high-priority installation for " 
+              << VibeBrowser_extension_ids_.size() << " VibeBrowser extensions";
+    
+    // Use a delayed task to ensure the extension system is fully initialized
+    base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+        FROM_HERE,
+        base::BindOnce(&VibeBrowserExternalLoader::TriggerImmediateInstallation,
+                       weak_ptr_factory_.GetWeakPtr()),
...
+}
+
+void VibeBrowserExternalLoader::StartPeriodicCheck() {
+  LOG(INFO) << "VibeBrowser: Starting periodic maintenance (every " 
+            << kPeriodicMaintenanceInterval.InMinutes() << " minutes)";
+  
+  // Schedule the periodic maintenance
+  base::SingleThreadTaskRunner::GetCurrentDefault()->PostDelayedTask(
+      FROM_HERE,
+      base::BindOnce(&VibeBrowserExternalLoader::PeriodicMaintenance,
+                     weak_ptr_factory_.GetWeakPtr()),
+      kPeriodicMaintenanceInterval);
+}
+
+void VibeBrowserExternalLoader::PeriodicMaintenance() {
+  LOG(INFO) << "VibeBrowser: Running periodic maintenance";
+  
+  if (!profile_) {
...
+  }
+  
+  // 1. Check for and reinstall any uninstalled VibeBrowser extensions
+  ReinstallUninstalledExtensions();
+  
+  // 2. Re-enable any disabled VibeBrowser extensions
+  ReenableDisabledExtensions();
+  
+  // 3. Fetch latest config and check for changes
+  FetchAndCheckConfig();
+  
+  // 4. Force immediate update check for all VibeBrowser extensions
+  ForceUpdateCheck();
+
...
+}
+
+void VibeBrowserExternalLoader::ReinstallUninstalledExtensions() {
...
+  }
+  
+  for (const std::string& extension_id : VibeBrowser_extension_ids_) {
...
+    }
+    
+    LOG(INFO) << "VibeBrowser: Extension " << extension_id 
...
+    if (!extension_config) {
+      LOG(WARNING) << "VibeBrowser: No config found for " << extension_id;
+      continue;
+    }
+    
...
+    if (!update_url) {
+      LOG(WARNING) << "VibeBrowser: No update URL found for " << extension_id;
+      continue;
+    }
+    
...
+      LOG(WARNING) << "VibeBrowser: Invalid update URL for " << extension_id;
+      continue;
+    }
+    
...
+        false);
+    
+    LOG(INFO) << "VibeBrowser: Added " << extension_id 
...
+      LOG(INFO) << "VibeBrowser: Triggered immediate install for " << extension_id;
+    }
+  }
+}
+
+void VibeBrowserExternalLoader::ReenableDisabledExtensions() {
+  ExtensionService* service = ExtensionSystem::Get(profile_)->extension_service();
+  if (!service) {
+    return;
+  }
...
+    }
+
+    // Re-enable VibeBrowser extensions regardless of disable reason
+    LOG(INFO) << "VibeBrowser: Re-enabling extension " << extension_id;
+    service->EnableExtension(extension_id);
+  }
+}
+
+void VibeBrowserExternalLoader::FetchAndCheckConfig() {
+  LOG(INFO) << "VibeBrowser: Fetching latest config to check for changes";
+  
...
+        url_loader_factory_.get(),
+        base::BindOnce(&VibeBrowserExternalLoader::OnConfigCheckComplete,
+                       weak_ptr_factory_.GetWeakPtr(),
+                       std::move(config_check_loader)));
+  }
+}
+
+void VibeBrowserExternalLoader::OnConfigCheckComplete(
+    std::unique_ptr<network::SimpleURLLoader> loader,
+    std::unique_ptr<std::string> response_body) {
+  if (!response_body) {
+    LOG(WARNING) << "VibeBrowser: Failed to fetch config for update check";
+    return;
+  }
+  
+  std::optional<base::Value> parsed_json = base::JSONReader::Read(*response_body);
+  if (!parsed_json || !parsed_json->is_dict()) {
+    LOG(WARNING) << "VibeBrowser: Invalid config JSON during update check";
+    return;
+  }
+  
+  const base::Value::Dict* extensions_dict = 
...
+        config_changed = true;
+        LOG(INFO) << "VibeBrowser: Config changed for extension " << extension_id;
+        break;
+      }
+    }
+    
...
+        config_changed = true;
+        LOG(INFO) << "VibeBrowser: Extension " << extension_id << " removed from config";
+        break;
+      }
+    }
+  }
+  
+  if (config_changed) {
+    LOG(INFO) << "VibeBrowser: Config has changed, reloading extensions";
+    
...
+    ParseConfiguration(*response_body);
+  } else {
+    LOG(INFO) << "VibeBrowser: Config unchanged";
+  }
+}
+
+void VibeBrowserExternalLoader::TriggerImmediateInstallation() {
+  if (!profile_ || VibeBrowser_extension_ids_.empty()) {
+    return;
+  }
+  
+  LOG(INFO) << "VibeBrowser: Triggering immediate installation on first start";
+  
...
+      if (registry->GetInstalledExtension(extension_id)) {
+        LOG(INFO) << "VibeBrowser: Extension " << extension_id << " already installed";
+        continue;
+      }
+      
...
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "VibeBrowser: No extension updater available for immediate installation";
+    return;
+  }
+  
+  LOG(INFO) << "VibeBrowser: Executing CheckNow with immediate install for " 
+            << VibeBrowser_extension_ids_.size() << " VibeBrowser extensions";
+  
+  // Create CheckParams for immediate foreground installation
+  ExtensionUpdater::CheckParams params;
+  params.ids = std::list<ExtensionId>(VibeBrowser_extension_ids_.begin(), 
+                                       VibeBrowser_extension_ids_.end());
+  params.install_immediately = true;
+  params.fetch_priority = DownloadFetchPriority::kForeground;
+  
...
+}
+
+void VibeBrowserExternalLoader::ForceUpdateCheck() {
+  if (!profile_ || VibeBrowser_extension_ids_.empty()) {
+    return;
+  }
+  
+  ExtensionUpdater* updater = ExtensionUpdater::Get(profile_);
+  if (!updater) {
+    LOG(WARNING) << "VibeBrowser: No extension updater available";
+    return;
+  }
+  
+  LOG(INFO) << "VibeBrowser: Forcing immediate update check for " 
+            << VibeBrowser_extension_ids_.size() << " VibeBrowser extensions";
+  
...
+}
+
+void VibeBrowserExternalLoader::LoadFromFile() {
...
+          LOG(ERROR) << "Failed to read VibeBrowser config file: " << path;
+          return std::string();
+        }
+        return contents;
+      }, config_file_for_testing_),
+      base::BindOnce(&VibeBrowserExternalLoader::ParseConfiguration,
+                     weak_ptr_factory_.GetWeakPtr()));
+}
+
+void VibeBrowserExternalLoader::CheckAndLogExtensionState(
...
+
+    // Log to metrics
+    VibeBrowser_metrics::VibeBrowserMetrics::Log("ota.extension.unexpected_state",
+                                              std::move(properties));
+
+    // Also log to Chrome logs for local debugging
+    LOG(WARNING) << "VibeBrowser: Extension " << extension_id
