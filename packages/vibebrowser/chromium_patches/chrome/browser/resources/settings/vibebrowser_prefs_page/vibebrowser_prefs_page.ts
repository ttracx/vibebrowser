diff --git a/chrome/browser/resources/settings/VibeBrowser_prefs_page/VibeBrowser_prefs_page.ts b/chrome/browser/resources/settings/VibeBrowser_prefs_page/VibeBrowser_prefs_page.ts
new file mode 100644
index 0000000000000..2616c77cc001b
--- /dev/null
+++ b/chrome/browser/resources/settings/VibeBrowser_prefs_page/VibeBrowser_prefs_page.ts
@@ -0,0 +1,250 @@
...
+
+/**
+ * @fileoverview 'settings-VibeBrowser-prefs-page' contains VibeBrowser-specific settings.
+ */
+
...
+
+import {getTemplate} from './VibeBrowser_prefs_page.html.js';
+
...
+}
+
+export interface SettingsVibeBrowserPrefsPageElement {
+  $: {
+    addProviderDialog: HTMLElement;
+  };
+}
+
+const SettingsVibeBrowserPrefsPageElementBase = PrefsMixin(PolymerElement);
+
+export class SettingsVibeBrowserPrefsPageElement extends SettingsVibeBrowserPrefsPageElementBase {
+  static get is() {
+    return 'settings-VibeBrowser-prefs-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    try {
+      const pref = this.getPref('VibeBrowser.custom_providers');
+      if (pref && pref.value) {
...
+    this.setPrefValue('VibeBrowser.custom_providers', customProvidersJson);
+  }
+  
+  /**
...
+    'settings-VibeBrowser-prefs-page': SettingsVibeBrowserPrefsPageElement;
+  }
+}
+
+customElements.define(
+    SettingsVibeBrowserPrefsPageElement.is, SettingsVibeBrowserPrefsPageElement);
