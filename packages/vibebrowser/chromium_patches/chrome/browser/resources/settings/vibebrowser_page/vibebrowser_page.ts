diff --git a/chrome/browser/resources/settings/vibebrowser_page/vibebrowser_page.ts b/chrome/browser/resources/settings/vibebrowser_page/vibebrowser_page.ts
new file mode 100644
index 0000000000000..80e988de4d595
--- /dev/null
+++ b/chrome/browser/resources/settings/vibebrowser_page/vibebrowser_page.ts
@@ -0,0 +1,1099 @@
...
+
+/**
+ * @fileoverview 'settings-vibebrowser-page' contains AI provider-specific settings.
+ */
+
...
+
+import {getTemplate} from './vibebrowser_page.html.js';
+import {MODELS_DATA, getModelsForProvider, getModelContextLength} from './models_data.js';
+
+const SettingsVibeBrowserPageElementBase = PrefsMixin(PolymerElement);
+
+export enum ProviderType {
+  VibeBrowser = 'VibeBrowser',
+  OPENAI = 'openai',
...
+}
+
+export class SettingsVibeBrowserPageElement extends SettingsVibeBrowserPageElementBase {
+  static get is() {
+    return 'settings-vibebrowser-page';
+  }
+
+  static get template() {
+    return getTemplate();
...
+    if (this.prefs && this.prefs.VibeBrowser) {
+      this.loadProviders_();
+    }
+  }
+
+  private loadProviders_() {
+    // Load from preferences or initialize with VibeBrowser
+    if (!this.prefs || !this.prefs.VibeBrowser || !this.prefs.VibeBrowser.providers) {
+      // Prefs not ready yet, initialize defaults locally only
+      this.initializeDefaultProvidersLocally_();
+      return;
+    }
+    
+    const stored = this.getPref('VibeBrowser.providers');
...
+        
+        // Ensure VibeBrowser is always present
+        if (!this.providers_.some(p => p.id === 'VibeBrowser')) {
...
+    
+    const VibeBrowser: ProviderConfig = {
+      id: 'VibeBrowser',
+      name: 'VibeBrowser',
+      type: ProviderType.VibeBrowser,
+      isDefault: true,
+      isBuiltIn: true,
+      createdAt: now,
+      updatedAt: now,
+    };
+    
+    this.providers_ = [VibeBrowser];
+    this.defaultProviderId_ = 'VibeBrowser';
...
+    if (this.prefs && this.prefs.VibeBrowser && this.prefs.VibeBrowser.providers !== undefined) {
+      this.saveProviders_();
+    }
+  }
+
...
+    if (!this.prefs || !this.prefs.VibeBrowser || this.prefs.VibeBrowser.providers === undefined) {
+      console.warn('VibeBrowser: Cannot save providers - prefs not ready');
+      return;
+    }
+    
...
+    this.setPrefValue('VibeBrowser.providers', JSON.stringify(data));
+    console.log('VibeBrowser: Saving providers:', data);
+  }
+
...
+    console.log('VibeBrowser: Add Provider clicked');
+    
...
+    }
+    
+    console.log('VibeBrowser: Form visibility:', this.showProviderForm_);
+  }
+
...
+    const provider = this.providers_.find(p => p.id === providerId);
+    
+    console.log('VibeBrowser: Delete provider clicked:', providerId, provider);
+    
...
+      
+      // If deleted provider was default, set VibeBrowser as default
+      if (provider.id === this.defaultProviderId_) {
+        this.set('defaultProviderId_', 'VibeBrowser');
...
+    }
+    
+    console.log('VibeBrowser: Provider type changed to:', providerType);
+    
+    const defaults = PROVIDER_DEFAULTS[providerType];
+    if (defaults) {
+      console.log('VibeBrowser: Applying defaults:', defaults);
...
+            console.log('VibeBrowser: Auto-set context length to:', contextLength);
+          }
+          
+          this.set('showModelDropdown_', false);
+          this.set('selectedSuggestionIndex_', -1);
...
+    console.log('VibeBrowser: Model suggestion clicked:', model);
+    
+    if (model) {
+      console.log('VibeBrowser: Setting dialogModelId_ to:', model);
...
+      if (input) {
+        input.value = model;
+        console.log('VibeBrowser: Input value set to:', input.value);
+      }
+    }
+  }
+  
...
+    console.log('VibeBrowser: Custom model selected:', this.dialogModelId_);
+    
...
+      
+      chrome.send('logVibeBrowserMetric', ['settings.provider.added', {
...
+    console.log('VibeBrowser: Validating form:', {
+      name: this.dialogProviderName_,
+      type: this.dialogProviderType_,
+      apiKey: this.dialogApiKey_,
+      modelId: this.dialogModelId_
...
+
+    chrome.send('logVibeBrowserMetric', ['settings.default_provider.changed', {
...
+      classes += ' selected';
+    }
+    if (provider.type === ProviderType.VibeBrowser) {
+      classes += ' VibeBrowser';
+    }
+    return classes;
+  }
...
+    }, 600);
+    
+    console.log('VibeBrowser: Using template:', template.name);
+  }
+
...
+    'settings-vibebrowser-page': SettingsVibeBrowserPageElement;
+  }
+}
+
+customElements.define(
+    SettingsVibeBrowserPageElement.is, SettingsVibeBrowserPageElement);
