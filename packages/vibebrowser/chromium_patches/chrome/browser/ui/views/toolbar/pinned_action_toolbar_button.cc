...
+#include "chrome/browser/ui/actions/VibeBrowser_actions_config.h"
...
+  // Update the text from the action item for VibeBrowser actions
+  if (VibeBrowser::IsVibeBrowserAction(action_view_->GetActionId())) {
+    // Check if labels should be shown
+    bool show_labels = true;
