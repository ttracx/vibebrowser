...
@@ -120,6 +122,14 @@ void ExtensionSidePanelManager::MaybeCreateActionItemForExtension(
           ...
           .Build());
+
+  // Auto-pin VibeBrowser extensions to the toolbar.
+  if (VibeBrowser::IsVibeBrowserExtension(extension->id())) {
+    LOG(INFO) << "VibeBrowser: Auto-pinning VibeBrowser extension: " << extension->id();
...
+  
+  // Unpin VibeBrowser extensions before removing the action item
+  if (VibeBrowser::IsVibeBrowserExtension(extension->id())) {
+    LOG(INFO) << "VibeBrowser: Unpinning VibeBrowser extension: " << extension->id() 
