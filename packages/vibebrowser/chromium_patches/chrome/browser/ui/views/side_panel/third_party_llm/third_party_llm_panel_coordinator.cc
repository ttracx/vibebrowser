...
+
+// Preference names
+const char kThirdPartyLlmProvidersPref[] = "VibeBrowser.third_party_llm.providers";
+const char kThirdPartyLlmSelectedProviderPref[] = "VibeBrowser.third_party_llm.selected_provider";
+
...
+    LOG(INFO) << "[VibeBrowser] Provider list size changed from " << previous_size
...
+  
+  VibeBrowser_metrics::VibeBrowserMetrics::Log("llmchat.created");
+  
+  return container;
+}
+
...
+  if (!prefs) {
+    LOG(ERROR) << "[VibeBrowser] Failed to get PrefService";
...
+      if (!item.is_dict()) {
+        LOG(WARNING) << "[VibeBrowser] Invalid provider entry (not a dict), skipping";
+        continue;
+      }
+
...
+
+      if (!name || name->empty()) {
+        LOG(WARNING) << "[VibeBrowser] Provider missing name, skipping";
+        continue;
+      }
+
+      if (!url || url->empty()) {
+        LOG(WARNING) << "[VibeBrowser] Provider missing URL, skipping";
+        continue;
+      }
+
+      GURL provider_url(*url);
+      if (!provider_url.is_valid()) {
+        LOG(WARNING) << "[VibeBrowser] Invalid provider URL: " << *url;
+        continue;
+      }
+
...
+  if (providers_.empty()) {
+    LOG(INFO) << "[VibeBrowser] No providers in prefs, using defaults";
+    providers_ = GetDefaultProviders();
...
+    LOG(WARNING) << "[VibeBrowser] Invalid selected provider index: " << selected_index
+                 << ", resetting to 0";
+    current_provider_index_ = 0;
+    prefs->SetInteger(kThirdPartyLlmSelectedProviderPref, 0);
+  } else {
...
+  PrefService* prefs = GetBrowser().profile()->GetPrefs();
+  if (!prefs) {
+    LOG(ERROR) << "[VibeBrowser] Failed to get PrefService for saving";
+    return;
+  }
+
...
+    LOG(ERROR) << "[VibeBrowser] Invalid provider index: " << new_provider_index;
+    return;
+  }
+
+  provider_change_in_progress_ = true;
+
+  VibeBrowser_metrics::VibeBrowserMetrics::Log("llmchat.provider.changed");
+
+  if (owned_web_contents_) {
+    GURL current_url = owned_web_contents_->GetURL();
+    if (current_url.is_valid()) {
...
+  
+  VibeBrowser_metrics::VibeBrowserMetrics::Log("llmchat.screenshot.captured");
+
...
+    
+    VibeBrowser_metrics::VibeBrowserMetrics::Log("llmchat.content.copied");
+    
...
+  LOG(INFO) << "[VibeBrowser] Media access requested for origin: "
...
+
+    LOG(INFO) << "[VibeBrowser] Auto-granting media permission for trusted LLM: "
+              << origin_url.host();
+
...
+      VLOG(2) << "[VibeBrowser] Media permission check for trusted LLM origin: "
...
+
+  VLOG(2) << "[VibeBrowser] Media permission check for origin: "
+          << security_origin.Serialize()
+          << " type: " << static_cast<int>(type)
