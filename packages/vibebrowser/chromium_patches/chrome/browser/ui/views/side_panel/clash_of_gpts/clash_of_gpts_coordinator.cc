...
+
+// Preference names
+const char kClashOfGptsPaneProvidersPref[] = "VibeBrowser.clash_of_gpts.pane_providers";  // Per-pane selections
+const char kClashOfGptsLastUrlsPref[] = "VibeBrowser.clash_of_gpts.last_urls";
+const char kClashOfGptsPaneCountPref[] = "VibeBrowser.clash_of_gpts.pane_count";
+
...
+    widget_->Show();
+    widget_->Activate();
+    VibeBrowser_metrics::VibeBrowserMetrics::Log("llmhub.shown");
+  }
+}
+
...
+    LOG(ERROR) << "[VibeBrowser] Invalid provider index: " << provider_index;
+    return;
+  }
+
...
+  SaveState();
+  
+  VibeBrowser_metrics::VibeBrowserMetrics::Log("llmhub.panecount.changed", 
