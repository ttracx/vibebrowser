...
+  
+  // Don't show the dialog for VibeBrowser extensions
+  for (const char* allowed_id : extensions::VibeBrowser::kAllowedExtensions) {
+    if (extension->id() == allowed_id) {
+      LOG(INFO) << "VibeBrowser: Skipping settings override dialog for VibeBrowser extension " 
+                << extension->id();
+      return std::nullopt;
+    }
+  }
