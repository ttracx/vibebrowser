diff --git a/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics_service.h b/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics_service.h
new file mode 100644
index 0000000000000..cb7519c887826
--- /dev/null
+++ b/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics_service.h
@@ -0,0 +1,95 @@
+// Copyright 2025 The Chromium Authors
+// Use of this source code is governed by a BSD-style license that can be
+// found in the LICENSE file.
+
+#ifndef COMPONENTS_METRICS_VibeBrowser_METRICS_VibeBrowser_METRICS_SERVICE_H_
+#define COMPONENTS_METRICS_VibeBrowser_METRICS_VibeBrowser_METRICS_SERVICE_H_
+
+#include <memory>
+#include <string>
+
...
+class VibeBrowserMetricsService : public KeyedService {
+ public:
+  explicit VibeBrowserMetricsService(
+      PrefService* pref_service,
+      PrefService* local_state_prefs,
+      scoped_refptr<network::SharedURLLoaderFactory> url_loader_factory);
+
+  VibeBrowserMetricsService(const VibeBrowserMetricsService&) = delete;
+  VibeBrowserMetricsService& operator=(const VibeBrowserMetricsService&) = delete;
+
+  ~VibeBrowserMetricsService() override;
+
...
+  base::WeakPtrFactory<VibeBrowserMetricsService> weak_factory_{this};
+};
+
+}  // namespace VibeBrowser_metrics
+
+#endif  // COMPONENTS_METRICS_VibeBrowser_METRICS_VibeBrowser_METRICS_SERVICE_H_
