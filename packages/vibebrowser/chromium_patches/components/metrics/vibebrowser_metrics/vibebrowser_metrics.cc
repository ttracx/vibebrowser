diff --git a/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics.cc b/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics.cc
new file mode 100644
index 0000000000000..02a1dc121bb7a
--- /dev/null
+++ b/components/metrics/VibeBrowser_metrics/VibeBrowser_metrics.cc
@@ -0,0 +1,100 @@
...
+VibeBrowserMetricsService* GetMetricsService() {
...
+  return VibeBrowserMetricsServiceFactory::GetForBrowserContext(profile);
+}
+
...
+  } else {
+    VLOG(1) << "VibeBrowser: Metrics service not available for event: " << event_name;
+  }
+}
+
+}  // namespace
+
+// static
+void VibeBrowserMetrics::Log(const std::string& event_name, double sample_rate) {
+  Log(event_name, base::Value::Dict(), sample_rate);
+}
+
+// static
+void VibeBrowserMetrics::Log(const std::string& event_name,
...
+
+// static
+void VibeBrowserMetrics::Log(const std::string& event_name, base::Value::Dict properties,
+                           double sample_rate) {
