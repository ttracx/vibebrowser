#!/usr/bin/env python3
"""
Windows packaging module for VibeBrowser Browser
...
# VibeBrowser Server binaries packaged alongside Chrome that must be signed prior to
# building the installer. Extend this list when new server-side executables are added.
VibeBrowser_SERVER_BINARIES: List[str] = [
    "VibeBrowser_server.exe",
    "codex.exe",
]


def get_VibeBrowser_server_binary_paths(build_output_dir: Path) -> List[Path]:
    """Return absolute paths to VibeBrowser Server binaries for signing."""
    server_dir = build_output_dir / "VibeBrowserServer" / "default"
    return [server_dir / binary for binary in VibeBrowser_SERVER_BINARIES]
...
def create_installer(ctx: BuildContext) -> bool:
    ...
    installer_name = f"{ctx.get_app_base_name()}_{ctx.get_vibebrowser_chromium_version()}_{ctx.architecture}_installer.exe"
...
def create_portable_zip(ctx: BuildContext) -> bool:
    ...
    zip_name = f"{ctx.get_app_base_name()}_{ctx.get_vibebrowser_chromium_version()}_{ctx.architecture}_installer.zip"
    ...
    try:
        with zipfile.ZipFile(zip_path, "w", zipfile.ZIP_DEFLATED) as zipf:
            ...
            installer_name = f"{ctx.get_app_base_name()}_{ctx.get_vibebrowser_version()}_{ctx.architecture}_installer.exe"
...
def sign_binaries(ctx: BuildContext, certificate_name: Optional[str] = None) -> bool:
    ...
    # STEP 1: Sign chrome.exe and VibeBrowser Server binaries BEFORE building mini_installer
    log_info("\nStep 1/3: Signing executables before packaging...")
    binaries_to_sign_first = [build_output_dir / "chrome.exe"]
    binaries_to_sign_first.extend(get_VibeBrowser_server_binary_paths(build_output_dir))
