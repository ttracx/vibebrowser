#!/usr/bin/env python3
"""
Application signing and notarization module for VibeBrowser
...
# Central list of VibeBrowser Server binaries we need to sign explicitly.
...
VibeBrowser_SERVER_BINARIES: Dict[str, Dict[str, str]] = {
    "VibeBrowser_server": {
        "identifier_suffix": "VibeBrowser_server",
        "options": "runtime",
        "entitlements": "VibeBrowser-executable-entitlements.plist",
    },
    "codex": {
...
def get_VibeBrowser_server_binary_info(component_path: Path) -> Optional[Dict[str, str]]:
    """Return metadata for known VibeBrowser Server binaries, if applicable."""
    name = component_path.stem.lower()
    return VibeBrowser_SERVER_BINARIES.get(name)
...
def find_components_to_sign(
    ...
    # Check both versioned and non-versioned paths for VibeBrowser Framework
    # Handle both release and debug framework names
    framework_names = [
        "VibeBrowser Framework.framework",
        "VibeBrowser Dev Framework.framework",
    ]
    vibebrowser_framework_paths = []

    for fw_name in framework_names:
        fw_path = join_paths(framework_path, fw_name)
        if fw_path.exists():
            vibebrowser_framework_paths.append(fw_path)

            # Add versioned path if context is available
            if ctx and ctx.vibebrowser_chromium_version:
                versioned_path = join_paths(
                    fw_path, "Versions", ctx.vibebrowser_chromium_version
                )
                if versioned_path.exists():
                    vibebrowser_framework_paths.insert(
                        0, versioned_path
                    )  # Prioritize versioned path

    # Find all helper apps
    for vibebrowser_fw_path in vibebrowser_framework_paths:
        helpers_dir = join_paths(vibebrowser_fw_path, "Helpers")
    ...
    # Find all dylibs (check versioned path for VibeBrowser Framework libraries)
    for vibebrowser_fw_path in vibebrowser_framework_paths:
        libraries_dir = join_paths(vibebrowser_fw_path, "Libraries")
    ...
    # Find VibeBrowser Server binaries
    VibeBrowser_server_dir = join_paths(app_path, "Contents", "Resources", "VibeBrowserServer")
    if VibeBrowser_server_dir.exists():
        for item in VibeBrowser_server_dir.rglob("*"):
            if item.is_file() and not item.suffix and os.access(item, os.X_OK):
                components["executables"].append(item)
...
def get_identifier_for_component(
    component_path: Path, base_identifier: str = "com.VibeBrowser"
) -> str:
    ...
    # VibeBrowser Server binaries share the same entitlements/options but need unique identifiers.
    VibeBrowser_server_info = get_VibeBrowser_server_binary_info(component_path)
    if VibeBrowser_server_info:
        suffix = VibeBrowser_server_info.get("identifier_suffix", component_path.stem)
    ...
    if component_path.suffix == ".framework":
        if name == "VibeBrowser Framework" or name == "VibeBrowser Dev Framework":
...
def get_signing_options(component_path: Path) -> str:
    ...
    # Known VibeBrowser Server binaries share the same relaxed options.
    VibeBrowser_server_info = get_VibeBrowser_server_binary_info(component_path)
    if VibeBrowser_server_info:
        return VibeBrowser_server_info.get("options", "runtime")
...
def sign_all_components(
    ...
    if components["executables"]:
        ...
        for exe in components["executables"]:
            ...
            entitlements = None
            VibeBrowser_server_info = get_VibeBrowser_server_binary_info(exe)
            if VibeBrowser_server_info:
                entitlements_name = VibeBrowser_server_info.get("entitlements")
    ...
    # 6. Sign frameworks (except the main VibeBrowser Framework)
    if components["frameworks"]:
        log_info("\n🔏 Signing frameworks...")
        # Sort to sign Sparkle.framework before VibeBrowser Framework.framework
    ...
    main_exe_names = ["VibeBrowser", "VibeBrowser Dev"]
    main_exe = None
    ...
    if not sign_component(main_exe, certificate_name, "com.VibeBrowser.VibeBrowser"):
        return False

    # 8. Finally sign the app bundle
    log_info("\n🔏 Signing application bundle...")
    requirements = (
        '=designated => identifier "com.VibeBrowser.VibeBrowser" and '
...
def sign_app(ctx: BuildContext, create_dmg: bool = True) -> bool:
    ...
    log_info("=" * 70)
    log_info("🚀 Starting signing process for VibeBrowser...")
    log_info("=" * 70)
...
def sign_universal(contexts: List[BuildContext]) -> bool:
    ...
    universal_app_path = join_paths(universal_dir, contexts[0].VIBEBROWSER_APP_NAME)
