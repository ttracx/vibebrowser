...
class BuildContext:
    ...
    vibebrowser_version: str = ""
    vibebrowser_chromium_version: str = ""
    start_time: float = 0.0

    # App names - will be set based on platform
    CHROMIUM_APP_NAME: str = ""
    VIBEBROWSER_APP_NAME: str = ""
    VIBEBROWSER_APP_BASE_NAME: str = "VibeBrowser"  # Base name without extension
    ...
    def __post_init__(self):
        ...
        if IS_WINDOWS:
            ...
            self.VIBEBROWSER_APP_NAME = (
                f"{self.VIBEBROWSER_APP_BASE_NAME}{get_executable_extension()}"
            )
        elif IS_MACOS:
            self.CHROMIUM_APP_NAME = "Chromium.app"
            self.VIBEBROWSER_APP_NAME = f"{self.VIBEBROWSER_APP_BASE_NAME}.app"
        else:
            self.CHROMIUM_APP_NAME = "chrome"
            self.VIBEBROWSER_APP_NAME = self.VIBEBROWSER_APP_BASE_NAME.lower()
        ...
        if not self.vibebrowser_version:
            # Read from VIBEBROWSER_VERSION file
            version_file = join_paths(
                self.root_dir, "build", "config", "VIBEBROWSER_VERSION"
            )
            if version_file.exists():
                self.vibebrowser_version = version_file.read_text().strip()

        # Set vibebrowser_chromium_version as chromium version with BUILD + vibebrowser_version
        if self.chromium_version and self.vibebrowser_version and version_dict:
            # Calculate new BUILD number by adding vibebrowser_version to original BUILD
            new_build = int(version_dict["BUILD"]) + int(self.vibebrowser_version)
            self.vibebrowser_chromium_version = f"{version_dict['MAJOR']}.{version_dict['MINOR']}.{new_build}.{version_dict['PATCH']}"
    ...
    def get_vibebrowser_patches_dir(self) -> Path:
        """Get VibeBrowser specific patches directory"""
        return join_paths(self.get_patches_dir(), "VibeBrowser")
    ...
    def get_app_path(self) -> Path:
        ...
        if self.build_type == "debug" and IS_MACOS:
            ...
            debug_app_name = f"{self.VIBEBROWSER_APP_BASE_NAME} Dev.app"
            debug_app_path = join_paths(self.chromium_src, self.out_dir, debug_app_name)
            if debug_app_path.exists():
                return debug_app_path
        return join_paths(self.chromium_src, self.out_dir, self.VIBEBROWSER_APP_NAME)
    ...
    def get_dmg_name(self, signed=False) -> str:
        ...
        if self.architecture == "universal":
            if signed:
                return f"{self.VIBEBROWSER_APP_BASE_NAME}_{self.vibebrowser_chromium_version}_universal_signed.dmg"
            return f"{self.VIBEBROWSER_APP_BASE_NAME}_{self.vibebrowser_chromium_version}_universal.dmg"
        else:
            if signed:
                return f"{self.VIBEBROWSER_APP_BASE_NAME}_{self.vibebrowser_chromium_version}_{self.architecture}_signed.dmg"
            return f"{self.VIBEBROWSER_APP_BASE_NAME}_{self.vibebrowser_chromium_version}_{self.architecture}.dmg"

    def get_vibebrowser_chromium_version(self) -> str:
        """Get VibeBrowser version string"""
        return self.vibebrowser_chromium_version

    def get_vibebrowser_version(self) -> str:
        """Get VibeBrowser version string"""
        return self.vibebrowser_version

    def get_app_base_name(self) -> str:
        """Get app base name without extension"""
        return self.VIBEBROWSER_APP_BASE_NAME

    def get_dist_dir(self) -> Path:
        """Get distribution output directory with version"""
        return join_paths(self.root_dir, "dist", self.vibebrowser_version)
