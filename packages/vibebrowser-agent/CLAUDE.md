...
- OUR PRODUCT NAME IS VibeBrowser -- so use that name correctly when naming things.
...
- **Provider Strategies**: AnthropicStrategy, OpenAIStrategy, OllamaStrategy, VibeBrowser Strategy
...
```
User Query → VibeBrowser.run() → BrowserAgent.execute()
                                        ↓
                              ClassificationTool
                                   ↙        ↘
