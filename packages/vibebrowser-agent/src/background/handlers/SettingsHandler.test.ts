...
// Mock Chrome VibeBrowser prefs API
const mockVibeBrowserPrefs = {
  getPrefs: vi.fn(),
  setPrefs: vi.fn()
}
...
describe('SettingsHandler-unit-test', () => {
  ...
  beforeEach(() => {
    ...
    ;(globalThis as any).chrome = {
      VibeBrowser: mockVibeBrowserPrefs
    }
  })
  ...
  it('tests that handleGetPref calls chrome.VibeBrowser.getPrefs correctly', async () => {
    mockVibeBrowserPrefs.getPrefs.mockImplementation((keys: string[], callback) => {
    ...
    expect(mockVibeBrowserPrefs.getPrefs).toHaveBeenCalledWith(['test-pref'], expect.any(Function))
  ...
  it('tests that handleSetPref calls chrome.VibeBrowser.setPrefs correctly', async () => {
    mockVibeBrowserPrefs.setPrefs.mockImplementation((prefs: Record<string, unknown>, callback) => {
      callback?.(true)
    })
    ...
    // Verify browserOS API was called
    expect(mockVibeBrowserPrefs.setPrefs).toHaveBeenCalledWith({ 'test-pref': 'test-value' }, expect.any(Function))
  ...
  it('tests that error handling works when chrome APIs throw', async () => {
    mockVibeBrowserPrefs.getPrefs.mockImplementation(() => {
