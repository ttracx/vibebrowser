...
import { VibeBrowserProvidersConfigSchema, BROWSEROS_PREFERENCE_KEYS } from '@/lib/llm/settings/browserOSTypes'
...
export class ProvidersHandler {
  ...
  async handleSaveProviders(
    ...
    try {
      ...
      const config = VibeBrowserProvidersConfigSchema.parse(payload)
      ...
      if (browserOS?.setPref) {
        browserOSSuccess = await new Promise<boolean>((resolve) => {
          browserOS.setPref(key, configStr, undefined, (success: boolean) => {
            ...
            if (error) {
              Logging.log('ProvidersHandler', `VibeBrowser setPref error: ${error.message}`, 'warning')
              resolve(false)
            } else if (success) {
              resolve(true)
            } else {
              Logging.log('ProvidersHandler', 'VibeBrowser setPref returned false', 'warning')
              resolve(false)
            }
          })
        })
