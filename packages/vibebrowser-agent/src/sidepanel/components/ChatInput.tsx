...
import { VibeBrowserProvidersConfig, VibeBrowserProvider } from '@/lib/llm/settings/browserOSTypes'
...
export function ChatInput({ isConnected, isProcessing }: ChatInputProps) {
  ...
  useEffect(() => {
    const computeOk = (cfg: VibeBrowserProvidersConfig) => {
    ...
    const handleWorkflow = (payload: any) => {
      ...
      if (payload && payload.data && payload.data.providersConfig) {
        computeOk(payload.data.providersConfig as VibeBrowserProvidersConfig)
      }
    }
  ...
  const isProviderUsable = (provider: VibeBrowserProvider | null): boolean => {
