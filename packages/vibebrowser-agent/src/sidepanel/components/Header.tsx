...
import { z } from 'zod'
import { VibeBrowserProvidersConfig, VibeBrowserProvidersConfigSchema } from '@/lib/llm/settings/types'
import { MCP_SERVERS, type MCPServerConfig } from '@/config/mcpServers'

const GITHUB_REPO_URL: string = 'https://github.com/vibebrowser-ai/VibeBrowser'
const DOCS_URL: string = 'https://docs.vibebrowser.com/'
...
export const Header = memo(function Header({ onReset, showReset, isProcessing, isTeachMode = false }: HeaderProps) {
  ...
  const [providersConfig, setProvidersConfig] = useState<VibeBrowserProvidersConfig | null>(null)
    ...
    try {
      ...
      try {
        await chrome.tabs.create({
          url: chrome.runtime.getURL('vibebrowser-settings.html')
        })
      } catch (fallbackError) {
    ...
    const handler = (payload: any) => {
      if (payload && payload.status === 'success' && payload.data) {
        ...
        if (payload.data.providersConfig) {
          try {
            const cfg = VibeBrowserProvidersConfigSchema.parse(payload.data.providersConfig)
