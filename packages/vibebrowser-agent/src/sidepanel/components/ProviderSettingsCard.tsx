...
export function ProviderSettingsCard(): JSX.Element | null {
  ...
  const [config, setConfig] = useState<VibeBrowserProvidersConfig | null>(null)
  ...
  useEffect(() => {
    const handler = (payload: any) => {
      if (payload && payload.status === 'success' && payload.data && payload.data.providersConfig) {
        const cfg = payload.data.providersConfig as VibeBrowserProvidersConfig
        setConfig(cfg)
  ...
  const onSelectProvider = (providerId: string) => {
    ...
    const nextConfig: VibeBrowserProvidersConfig = {
      defaultProviderId: providerId,
      providers: nextProviders
    }
    try {
      VibeBrowserProvidersConfigSchema.parse(nextConfig)
      setConfig(nextConfig)
      setSelectedProviderId(providerId)
      const ok = sendMessage<VibeBrowserProvidersConfig>(MessageType.SAVE_LLM_PROVIDERS as any, nextConfig)
  ...
  const onEditProvider = (provider: VibeBrowserProvider) => {
    setError(null)
    setDraftOpen(true)
  ...
  const toProvider = (p: ProviderDraft, isDefault: boolean): VibeBrowserProvider => {
    ...
    return {
      id: p.id,
      name: p.name,
      type: p.type as VibeBrowserProviderType,
      isDefault,
      isBuiltIn: false,
  ...
  const onSave = () => {
    ...
    try {
      ...
      let nextProviders: VibeBrowserProvider[]
        ...
        <div className="space-y-2">
          {config.providers.map(p => (
            <div key={p.id} className="flex items-center justify-between gap-2">
              ...
              <div className="flex items-center gap-2">
                {!(p.isBuiltIn || p.id === 'vibebrowser' || p.type === 'vibebrowser') && (
                  <>
                    <Button
                      size="sm"
                      variant="ghost"
          ...
          <div className="mt-3 grid grid-cols-2 gap-3">
            <div className="flex flex-col">
              ...
              <select
                ...
                className="px-2 py-1 rounded-md border border-border bg-background text-sm"
                value={draft.type}
                onChange={e => setDraft({ ...draft, type: e.target.value as VibeBrowserProviderType })}
              >
