...
import { useVibeBrowserPrefs } from './hooks/useVibeBrowserPrefs'
...
export function OptionsNew() {
  const { providers, defaultProvider, setDefaultProvider, addProvider, updateProvider, deleteProvider } = useVibeBrowserPrefs()
  const { theme } = useSettingsStore()

  // Get initial section from URL hash or default to 'vibebrowser-ai'
  const getInitialSection = () => {
    const hash = window.location.hash.slice(1) // Remove '#'
    const validSections = ['vibebrowser-ai', 'providers-hub', 'mcp', 'search-providers', 'about']
    return validSections.includes(hash) ? hash : 'vibebrowser-ai'
  }
