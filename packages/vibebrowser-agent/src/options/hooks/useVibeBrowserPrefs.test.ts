...
import { useVibeBrowserPrefs } from './useVibeBrowserPrefs'
...
describe('useVibeBrowserPrefs-unit-test', () => {
  it('tests that hook can be initialized with default state', () => {
    const { result } = renderHook(() => useVibeBrowserPrefs())

    expect(result.current.providers).toHaveLength(1)
    expect(result.current.providers[0].name).toBe('VibeBrowser')
    expect(result.current.providers[0].type).toBe('vibebrowser')
    expect(result.current.defaultProvider).toBe('vibebrowser')
    ...
    expect(mockPort.postMessage).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        payload: expect.objectContaining({
          name: 'vibebrowser.llm_providers'
        })
      })
    )
  })
...
describe('useVibeBrowserPrefs-integration-test', () => {
  it.skipIf(!process.env.LITELLM_API_KEY || process.env.LITELLM_API_KEY === 'nokey')(
    ...
    async () => {
      ...
      console.log('✅ useVibeBrowserPrefs integration test passed')
    },
    30000
  )
})