...
import { VibeBrowserProvidersConfig } from '@/lib/llm/settings/browserOSTypes'
...
export function useVibeBrowserPrefs() {
  const [providers, setProviders] = useState<LLMProvider[]>([DEFAULT_BROWSEROS_PROVIDER])
  const [defaultProvider, setDefaultProviderState] = useState<string>('vibebrowser')
  ...
  const ensurePortConnected = useCallback(() => {
    if (!port || !isPortConnected) {
      console.warn('[useVibeBrowserPrefs] Port not connected')
      return null
    }
    return port
  ...
  useEffect(() => {
    ...
    const setupPort = () => {
      try {
        ...
        messageListener = (msg: PortMessage) => {
          ...
          if (msg.type === MessageType.WORKFLOW_STATUS) {
            ...
            if (payload?.status === 'error') {
              console.error('[useVibeBrowserPrefs] Error from background:', payload.error)
            }
            if (payload?.data?.providersConfig) {
              const config = payload.data.providersConfig as VibeBrowserProvidersConfig
              ...
              setDefaultProviderState(config.defaultProviderId || 'vibebrowser')
              setIsLoading(false)
            }
          }
        }
        ...
        disconnectListener = () => {
          console.warn('[useVibeBrowserPrefs] Port disconnected')
        ...
        heartbeatInterval = setInterval(() => {
          if (currentPort) {
            try {
              ...
              console.warn('[useVibeBrowserPrefs] Heartbeat failed, port likely disconnected')
              if (heartbeatInterval) {
                clearInterval(heartbeatInterval)
                heartbeatInterval = null
              }
        ...
        const initialTimeout = setTimeout(() => {
          if (currentPort) {
            try {
              ...
              console.error('[useVibeBrowserPrefs] Failed to send initial message:', error)
            }
          }
        }, 100)
        ...
        console.error('[useVibeBrowserPrefs] Failed to setup port:', error)
        setIsPortConnected(false)
        return () => {}
      }
    }
  ...
  const saveProvidersConfig = useCallback(async (updatedProviders: LLMProvider[], newDefaultId?: string) => {
    ...
    if (!connectedPort) {
      console.error('[useVibeBrowserPrefs] Port not connected, cannot save providers')
      return false
    }
    ...
    try {
      ...
      console.error('[useVibeBrowserPrefs] Failed to send save message:', error)
      setIsPortConnected(false)
      return false
    }
  ...
  const deleteProvider = useCallback(async (providerId: string) => {
    ...
    if (providerId === defaultProvider) {
      const browserOSProvider = remainingProviders.find(p => p.id === 'vibebrowser')
      nextDefaultId = browserOSProvider?.id || remainingProviders[0]?.id || 'vibebrowser'
