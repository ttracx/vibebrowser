...
import { VibeBrowserProvider } from '@/lib/llm/settings/browserOSTypes'
...
const BROWSEROS_DEFAULT_PROXY_URL = "https://llm.vibebrowser.com/default/"
const BROWSEROS_FAST_LLM_PROXY_URL = "https://llm.vibebrowser.com/fast/"
const BROWSEROS_SMART_LLM_PROXY_URL = "https://llm.vibebrowser.com/smart/"
const DEFAULT_BROWSEROS_MODEL_FAMILY_URL = "https://llm.vibebrowser.com/api/model_family"
...
// VibeBrowser API response structure
interface VibeBrowserModelConfig {
...
export class LangChainProvider {
  ...
  private currentProvider: VibeBrowserProvider | null = null
  ...
  // Model config cache for VibeBrowser - now stores full API response
  private modelConfigCache: { config: VibeBrowserModelConfig; timestamp: number } | null = null
  ...
  async getModelCapabilities(): Promise<ModelCapabilities> {
    ...
    if (provider.modelConfig?.contextWindow) {
      ...
      switch (provider.type) {
        case 'vibebrowser':
          // VibeBrowser/Nxtscape uses gemini 2.5 flash by default
          maxTokens = 1_000_000
          break
  ...
  getCurrentProvider(): VibeBrowserProvider | null {
  ...
  private async _createLLMFromProvider(
    ...
    switch (provider.type) {
      case 'vibebrowser':
        // Only VibeBrowser uses intelligence parameter
        const intelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createVibeBrowserLLM(temperature, maxTokens, streaming, intelligence)
      ...
      default:
        Logging.log('LangChainProvider',
          `Unknown provider type: ${provider.type}, falling back to VibeBrowser`,
          'warning')
        const defaultIntelligence = options?.intelligence ?? DEFAULT_INTELLIGENCE
        return await this._createVibeBrowserLLM(temperature, maxTokens, streaming, defaultIntelligence)
    }
  }
  
  // Fetch model configuration from VibeBrowser API
  private async _fetchModelConfig(): Promise<VibeBrowserModelConfig> {
    if (this.modelConfigCache) {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const config: VibeBrowserModelConfig = {
        ...
        Logging.log('LangChainProvider',
          `VibeBrowser model config fetched - default: ${config.default}, fast: ${config.fast}, smart: ${config.smart}`,
          'info')
        return config
      }
    } catch (error) {
    ...
    const fallbackConfig: VibeBrowserModelConfig = {
  ...
  // VibeBrowser built-in provider (uses proxy, no API key needed)
  private async _createVibeBrowserLLM(
    temperature: number,
    maxTokens?: number,
