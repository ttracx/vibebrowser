...
// Input schema for VibeBrowser info tool
const VibeBrowserInfoToolInputSchema = z.object({
  topic: z.enum([
    'overview',
    'installation',
    'configuration',
    ...
    'examples',
    'features'
  ]).describe('Information topic to retrieve about VibeBrowser'),
...
export type VibeBrowserInfoToolInput = z.infer<typeof VibeBrowserInfoToolInputSchema>

/**
 * VibeBrowserInfoTool - Provides comprehensive information about VibeBrowser features and usage
 * Contains various README-style documentation for different aspects of VibeBrowser
 */
export class VibeBrowserInfoToolImpl {
  ...
  async execute(input: VibeBrowserInfoToolInput): Promise<ToolOutput> {
    try {
      ...
      return toolError(`Failed to retrieve VibeBrowser info: ${error instanceof Error ? error.message : String(error)}`)
    }
  }

  private _getTopicInfo(topic: string, section?: string): string | null {
    const infoDatabase = {
      overview: `# VibeBrowser Overview
...
- AI-assisted web navigation`,
      ...
      installation: `# VibeBrowser Installation Guide
...
- Report issues with detailed reproduction steps`,
      ...
      examples: `# VibeBrowser Usage Examples
...
/**
 * Factory function to create VibeBrowserInfoTool for LangChain integration
 */
export function VibeBrowserInfoTool(executionContext: ExecutionContext): DynamicStructuredTool {
  const tool = new VibeBrowserInfoToolImpl(executionContext)

  return new DynamicStructuredTool({
    name: "vibebrowser_info_tool",
    description: `Get comprehensive information about VibeBrowser features, configuration, and usage.
    ...
    schema: VibeBrowserInfoToolInputSchema,
