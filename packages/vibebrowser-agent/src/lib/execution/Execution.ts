  ...
  async run(query: string, metadata?: ExecutionMetadata): Promise<void> {
    ...
    try {
      ...
      if (this.options.mode === "teach") {
        ...
        // Check if VibeBrowser provider is selected and websocket agent feature is enabled
        ...
        if (providerType === 'vibebrowser' && wsAgentEnabled) {
          // Use TeachWebSocketAgent for teach mode with VibeBrowser provider
        ...
        // Browse mode - check if VibeBrowser mode is enabled and websocket agent feature is enabled
        const providerType = await langChainProvider.getCurrentProviderType() || '';
        const featureFlags = getFeatureFlags();
        const wsAgentEnabled = featureFlags.isEnabled('WEBSOCKET_AGENT');

        // Use WebSocketAgent only when VibeBrowser provider is selected and feature is enabled
        if (providerType === 'vibebrowser' && wsAgentEnabled) {
      ...
      if (ENABLE_EVALS2 && evalsEventMgr.isEnabled()) {
        try {
          ...
          if (this.options.mode === 'chat') {
            ...
            const featureFlags = getFeatureFlags();
            const wsAgentEnabled = featureFlags.isEnabled('WEBSOCKET_AGENT');
            agentName = (providerType === 'vibebrowser' && wsAgentEnabled) ? 'TeachWebSocketAgent' : 'TeachAgent';
          } else {
