...
export const WSAgentConfigSchema = z.object({
  url: z.string().url(),  // Fallback URL if VibeBrowser preference not available
...
export class ExecutionContext {
  ...
  /**
   * Get WebSocket agent server URL from VibeBrowser preferences
   * Returns WebSocket URL using vibebrowser.server.agent_port preference value
   * Falls back to default URL from config if preference cannot be retrieved
   */
  public async getAgentServerUrl(): Promise<string> {
    const browserOS = getVibeBrowserAdapter();

    try {
      const pref = await browserOS.getPref('vibebrowser.server.agent_port');

      if (pref && typeof pref.value === 'number') {
        const wsUrl = `ws://localhost:${pref.value}`;
        Logging.log(
          "ExecutionContext",
          `Using agent port from VibeBrowser preferences: ${pref.value}`,
          "info"
        );
        return wsUrl;
      }
      ...
      Logging.log(
        "ExecutionContext",
        `Failed to get agent port from VibeBrowser preferences: ${error}, using default URL: ${WS_AGENT_CONFIG.url}`,
        "error"
      );
      return WS_AGENT_CONFIG.url;
    }
