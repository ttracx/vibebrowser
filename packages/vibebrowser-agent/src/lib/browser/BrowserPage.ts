...
export const InteractiveElementSchema = z.object({
  nodeId: z.number(), // Chrome VibeBrowser node ID (sequential index)
...
/**
 * BrowserPage - Simple browser page wrapper using Chrome VibeBrowser APIs
 *
 * This class provides:
 * 1. Direct element access via index-based APIs
 * 2. Element formatting for tools
 * 3. Simple action methods using VibeBrowserAdapter
 */
export class BrowserPage {
  private _tabId: number;
  private _url: string;
  private _title: string;
  private _browserOS = getVibeBrowserAdapter();
  ...
  async getHierarchicalText(): Promise<string> {
    // Get accessibility tree from VibeBrowser adapter
  ...
  async takeScreenshot(
    ...
    try {
      // Return the full data URL directly from VibeBrowser
  ...
  async takeScreenshotWithDimensions(
    ...
    try {
      // Return the full data URL directly from VibeBrowser with exact dimensions
      return await this._browserOS.captureScreenshot(
        this._tabId,
