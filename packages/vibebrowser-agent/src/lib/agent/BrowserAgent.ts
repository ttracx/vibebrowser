...
import { type ScreenshotSizeKey } from "@/lib/browser/VibeBrowserAdapter";
import {
  AIMessage,
  AIMessageChunk,
  HumanMessage,
...
export class BrowserAgent {
  ...
  private async _registerTools(): Promise<void> {
    ...
    this.toolManager.register(VibeBrowserInfoTool(this.executionContext)); // VibeBrowser info tool
  ...
  private _getSpecialTaskMetadata(task: string): {task: string, metadata: ExecutionMetadata} | null {
    ...
    // VibeBrowser Launch Upvote Task
    if (taskLower === "read about our vision and upvote ❤️") {
      return {
        ...
        metadata: {
          ...
          predefinedPlan: {
            agentId: 'vibebrowser-launch-upvoter',
            name: "VibeBrowser Launch Upvoter",
            goal: "Navigate to VibeBrowser launch page and upvote it",
            steps: [
              "Navigate to https://dub.sh/vibebrowser-launch",
    ...
    // GitHub Star Task
    if (taskLower === "support vibebrowser on github ⭐") {
      return {
        task: "Support VibeBrowser on GitHub",
        metadata: {
          executionMode: 'predefined' as const,
          predefinedPlan: {
            agentId: 'github-star-vibebrowser',
            name: "GitHub Repository Star",
            goal: "Navigate to VibeBrowser GitHub repo and star it",
            steps: [
...
${actionsToExecute}
  ...
  private _formatPlannerOutputForExecutor(plan: PlannerOutput | PredefinedPlannerOutput): string {
    return `VibeBrowser Agent Output:
- Task: ${plan.userTask}
- Current State: ${plan.currentState}
- Execution History: ${plan.executionHistory}
- Challenges Identified: ${plan.challengesIdentified}
