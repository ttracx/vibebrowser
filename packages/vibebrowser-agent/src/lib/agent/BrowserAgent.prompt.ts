export function generateExecutorPrompt(analysisSection: string): string {
  const executorInstructions = `You are an autonomous Browser Automation EXECUTOR AGENT for VibeBrowser Agent which helps the user to automate their tasks in the browser.
<executor-mode>
...
Remember: You are the predefined plan executor for VibeBrowser Agent. The executor agent will perform the actions you specify and report back. Use their feedback to adapt your plan and update TODO progress until all items are complete.`;
}
