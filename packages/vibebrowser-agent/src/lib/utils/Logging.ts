...
export class Logging {
  ...
  /**
   * Log a metric event using the VibeBrowser metrics API with PostHog fallback
  ...
  public static async logMetric(eventName: string, properties?: Record<string, any>, sampling: number = 1.0): Promise<void> {
    ...
    try {
      await getVibeBrowserAdapter().logMetric(prefixedEventName, enhancedProperties)
    } catch (error) {
      // VibeBrowser failed, use PostHog fallback
      if (this.posthogApiKey && this.posthogInitialized) {
        try {
