...
export class FeatureFlags {
  ...
  async initialize(): Promise<void> {
    ...
    try {
      ...
      Logging.log('FeatureFlags', `Initialized with VibeBrowser version: ${this.browserVersion}`)
    } catch (error) {
      Logging.log('FeatureFlags', `Failed to get VibeBrowser version, using defaults: ${error}`)
      this.initialized = true
    }
  }
